    ...
    for row in reader:
        ...
        if response != False:
            ...
            if soup.find('div',{'title':'Price'})!=None:
                price = soup.find('div',{'title':'Price'}).findNext('div',{'class':'infotile-text infotile-text-solo'}).text.rstrip().lstrip()
            ...
            gameCSV.writerow([row[0],translatedAppName,datePublished["content"],appSize,developer,price,installs,rating,int(ratingCount.replace(",","")),ranking,contentRating,androidVersions])
